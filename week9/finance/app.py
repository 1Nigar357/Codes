...
def index():
    ...
    if stocks:
        for stock in stocks:
            ...
            db.execute("UPDATE stocks SET price = ?, total_price = ? WHERE id = ? AND symbol = ?",
                       price["price"], total_price, session["user_id"], stock["symbol"])
    stocks = db.execute("SELECT * FROM stocks WHERE id = ?", session["user_id"])
    cash = db.execute("SELECT cash FROM users WHERE id = ?", session["user_id"])
    cash = cash[0]["cash"]
    total = cash
...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO history (id, symbol, shares, price, transaction_type, time) VALUES(?, ?, ?, ?, ?, ?)",
                   session["user_id"], symbol, shares, price, type, now)
        stock_symbol = db.execute("SELECT name, shares, symbol FROM stocks WHERE id = ? AND name = ?", session["user_id"], name)
        # If the user has never bought from that company a stock

        try:
            shares = stock_symbol[0]["shares"] + shares
            total_price1 = shares * price
            db.execute("UPDATE stocks SET shares = ?, total_price = ? WHERE id = ? and symbol = ?",
                       shares, total_price1, session["user_id"], symbol)
        except:
            db.execute("INSERT INTO stocks (id, symbol, name, shares, price, total_price) VALUES(?, ?, ?, ?, ?, ?)",
...
def sell():
    ...
    if request.method == "POST":
        ...
        if int(shares) == sell_stock[0]["shares"]:
            ...
            db.execute("UPDATE stocks SET shares = ?, total_price = ? WHERE id = ? AND symbol = ?",
                       current_shares, current_total_price, session["user_id"], symbol)
...
def change_password():
    ...
    if request.method == 'POST':
        ...
        if oldPassword == newPassword:
            return apology("your new password can't be your old password")
